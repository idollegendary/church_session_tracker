...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Fetch sessions in range with preacher info
    ...
    const top = Array.from(map.values()).sort((a, b) => b.total_seconds - a.total_seconds).slice(0, limit);

    return NextResponse.json({ top, range: { days, since } });
  } catch (err: any) {
